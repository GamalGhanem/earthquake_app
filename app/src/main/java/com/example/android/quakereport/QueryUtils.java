...
/**
 * Created by Gamal Ghanem on 8/9/2018.
 */
    ...
    public static ArrayList<Quake> extractEarthquakes() {
        ...
        try {
            ...
            for (int i = 0; i < quakes.length(); i ++) {
                JSONObject currentQuake = (JSONObject) quakes.get(i);
                JSONObject prop = (JSONObject) currentQuake.get("properties");
                double strength = prop.getDouble("mag");
                String location = prop.getString("place");
                long date = prop.getLong("time");
                String url = prop.getString("url");
                Quake newEarthquake = new Quake(strength, location, date, url);
